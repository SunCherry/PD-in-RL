...
class QN(object):
    ...
    """
    def __init__(self, env, config, logger=None, student=False, gen=0):
        """
    ...
    def train(self, exp_schedule, lr_schedule):
        ...
        self.save()
        scores_eval += [self.evaluate()]
        # plpath = self.config.model_output + "s%d_scores.png"% self.gen
        export_plot(scores_eval, "Scores", self.config.plot_output)
        # if not os.path.exists(plpath):
            # os.makedirs(plpath)
        # export_plot(scores_eval, "Scores", plpath)
