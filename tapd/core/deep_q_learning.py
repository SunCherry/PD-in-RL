...
class DQN(QN):
    ...
    def build_all(self, gen):
        """
        Build model by adding all necessary variables
        """
        # add placeholders
        # "gen" : ith generation, 0 denotes teacher

        self.add_placeholders_op()

        # compute Q values of state
        s = self.process_state(self.s)
        self.q = self.get_q_values_op(s, scope="q%d" % gen, reuse=False)
        # compute Q values of next state
        sp = self.process_state(self.sp)
        self.target_q = self.get_q_values_op(sp, scope="target_q%d" % gen, reuse=False)

        # add update operator for target network
        self.add_update_target_op("q%d" % gen, "target_q%d" % gen)

        # add square loss
        self.add_loss_op(self.q, self.target_q)

        # add optmizer for the main networks
        self.add_optimizer_op("q%d" % gen)
    ...
    def initialize_basic(self):
        ...
        if self.gen == 0:
            ...
            self.newlist = [v for v in tf.global_variables() if 'q%d'% (self.gen-1) not in v.name ]
            # and "beta1_power:0" not in v.name and "beta2_power:0" not in v.name ]# and "power_%d" % self.gen in v.name]
            # self.newlist= tf.global_variables(scope = )
            self.saver = tf.train.Saver(self.newlist)
        # self.saver = tf.train.Saver()
    ...
    def save(self):
        ...
        Saves session
        """
        ckpath = self.config.model_output + "s%d/" % self.gen + "model.weights/"
        # if not os.path.exists(self.config.model_output):
        #     os.makedirs(self.config.model_output)
        if not os.path.exists(ckpath):
            os.makedirs(ckpath)
        
        self.saver.save(self.sess, ckpath)
